...
import com.example.DigitalWayfinder.entity.RetailNonFunctional;

import java.util.List;

@Repository
public interface RetailNonFunctionalRepository extends JpaRepository<RetailNonFunctional, Long> {
    
    @Query(value = "SELECT L1, L2, L3 FROM [Retail_Non Fn]", nativeQuery = true)
    List<Object[]> findAllLevelsAsArray();

    @Query("SELECT DISTINCT w.l1 FROM RetailNonFunctional w WHERE w.l1 IS NOT NULL AND w.l1 != ''")
    List<String> findDistinctL1();
    
    @Query("SELECT DISTINCT w.l2 FROM RetailNonFunctional w WHERE w.l1 = :l1 AND w.l2 IS NOT NULL AND w.l2 != ''")
